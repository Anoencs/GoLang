...
func (cli *CommandLine) Update_okr_kr(dbname, tbname, create, last_modified, duedate, obj_id, name, id, user_id, last_modified_by, target_date, create_by string, itype, criterias uint64, start, target, self_grade, grade float64) *gorm.DB {
	...
	create_by_uuid, _ := uuid.Parse(create_by)
	last_modified_by_uuid, _ := uuid.Parse(last_modified_by)
	target_date_time, _ := time.Parse("00:00:00", target_date)
	...
	temp := models.Okr_kr{Id: id_uuid, Obj_id: obj_id_uuid, User_id: user_id_uuid, Name: name, Itype: itype, Criterias: criterias, Start: start, Target: target,
		Target_date: target_date_time, Self_grade: self_grade, Grade: grade, Duedate: duedate_time, Create: create_time, Create_by: create_by_uuid, Last_modified: last_modified_time, Last_modified_by: last_modified_by_uuid}
